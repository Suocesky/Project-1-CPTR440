def solution(input):
    ...
    # in the temper transform in reverse order. There are two kinds of operations in the temper 
    # transform each applied twice; one is an XOR against a right-shifted value, and the other is 
    # an XOR against a left-shifted value AND'd with a magic number. So you'll need code to invert the "right" and the "left" operation.
    # Once you have "untemper" working, create a new MT19937 generator, tap it for 624 outputs,
    #  untemper each of them to recreate the state of the generator, and splice that state into a new instance of the MT19937 generator.
